...
export const fetchIntraOralList = async () => {
    try {
        ...
        const params = { companyId: customerId, type};

        const response = await axios.get(`${API_BASE_URL}/api/configuration/companyConfig`, {
            headers: {
...
export const updateIntraOralList = async (data) => {
    try {
        const token = getAuthToken();
        const customerId = getCustomerId();
        if (!token) {
        ...
        const response = await axios.put(
            `${API_BASE_URL}/api/configuration/companyConfig`,
            {
        ...
        if (axios.isAxiosError(error)) {
            if (error.response?.status === 401) {
                ...
                try {
                    await refreshAuthToken();
                    return updateIntraOralList(data);
                } catch (refreshError) {
                    console.log('Token refresh failed:', refreshError);
                    throw new Error('Session expired. Please log in again.');
                }
